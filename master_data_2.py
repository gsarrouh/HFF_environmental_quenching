...
###in the tables: 
###     **"data_stats","macros","spec_stats","phot_stats","sample_stats","BCG_stats"
#
...
macs0416 = Table([z_macs0416['id'],z_macs0416['z_peak'],z_macs0416['z_spec'],F0416_u['L153'],F0416_v['L155'],F0416_j['L161'],F0416_u['DM'],f_macs0416['lmass'],f_macs0416['lsfr'],f_macs0416['lssfr'],cat_macs0416['flux_radius'],cat_macs0416['star_flag'],cat_macs0416['use_KS']], names=('id','z_peak','z_spec','u','v','j','DM','lmass','lsfr','lssfr','flux_radius','star_flag','BCG'))
...
while counter < size:
    if abell2744[counter]['z_spec'] > 0 and abell2744[counter]['z_peak'] > 0:  #entries w/ both spectroscopy and photometry  (SPECTROSCOPIC sub-sample)
        ...
        spec_only[0][5] +=1
    elif abell2744[counter]['z_spec'] < 0 and abell2744[counter]['z_peak'] > 0:  #entries w/ photometry (PHOTOMETRIC sub-sample)
        phot_only[0][5] +=1
        abell2744[counter]['sub'] = 2
    elif abell2744[counter]['z_spec'] < 0 and abell2744[counter]['z_peak'] < 0:
        nodata[0][5] +=1
...
#
#
### **Section 2: separate SF/Q galaxies for both phot & spec sub-samples:**
...
sp = Column([np.sum(both)-np.sum(spec_only)+outliers+star_type[0][1],np.sum(SF_spec),np.sum(Q_spec),star_type[0][1],outliers])
ph = Column([np.sum(phot_only),np.sum(SF_phot),np.sum(Q_phot),star_type[0][2],0])
macros_type = Table([macro_type,sp,ph],names=('Property','spec','phot'))
#
#
...
cl1 = Column([len(cat_macs0416),nodata[0][0],both[0][0],phot_only[0][0],star[0][0],SF_spec[0][0]+SF_phot[0][0],SF_spec[0][0],SF_phot[0][0],Q_spec[0][0]+Q_phot[0][0],Q_spec[0][0],Q_phot[0][0],out[0][0]])
cl2 = Column([len(cat_macs1149),nodata[0][1],both[0][1],phot_only[0][1],star[0][1],SF_spec[0][0]+SF_phot[0][0],SF_spec[0][0],SF_phot[0][0],Q_spec[0][1]+Q_phot[0][1],Q_spec[0][1],Q_phot[0][1],out[0][1]])
cl3 = Column([len(cat_macs0717),nodata[0][2],both[0][2]+spec_only[0][2],phot_only[0][2],star[0][2],SF_spec[0][0]+SF_phot[0][0],SF_spec[0][0],SF_phot[0][0],Q_spec[0][2]+Q_phot[0][2],Q_spec[0][2],Q_phot[0][2],out[0][2]])
cl4 = Column([len(cat_abell370),nodata[0][3],both[0][3],phot_only[0][3],star[0][3],SF_spec[0][0]+SF_phot[0][0],SF_spec[0][0],SF_phot[0][0],Q_spec[0][3]+Q_phot[0][3],Q_spec[0][3],Q_phot[0][3],out[0][3]])
cl5 = Column([len(cat_abell1063),nodata[0][4],both[0][4],phot_only[0][4],star[0][4],SF_spec[0][0]+SF_phot[0][0],SF_spec[0][0],SF_phot[0][0],Q_spec[0][4]+Q_phot[0][4],Q_spec[0][4],Q_phot[0][4],out[0][4]])
cl6 = Column([len(cat_abell2744),nodata[0][5],both[0][5],phot_only[0][5],star[0][5],SF_spec[0][0]+SF_phot[0][0],SF_spec[0][0],SF_phot[0][0],Q_spec[0][5]+Q_phot[0][5],Q_spec[0][5],Q_phot[0][5],out[0][5]])
...
BCG = Table(names=('cluster','sub','type','member','id','z_peak','z_spec','u','v','j','DM','lmass','lsfr','lssfr','flux_radius','star_flag','BCG','L_u','L_v','L_j','uv','vj','del_z','z_clusterspec','z_clusterphot'))
#
BCG_cl = [[0]*6]
BCG_sub= [[0]*4]
BCG_mem=[[0]*4]

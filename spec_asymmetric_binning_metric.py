...
#
    ...
    for ii in range(len(bin_edge_means_SF)):
        ...
        #
            SF_var_list = []
            if num_bins_SF[0][ii] < num_bins_SF[1][ii]:    # this if statement won't apply to the first/last bin, since they have been set to equal "range3", the mass range for the study
                if ii == 0:
                    ...
                    bin_edge_SF_end = max(num_bins_SF[1][ii],bin_edge_means_SF[(ii-1)])
                ...
                if ii == 0:
                    ...
                    bin_edge_SF_end = max(num_bins_SF[0][ii],bin_edge_means_SF[(ii-1)])
            ...
            if bin_edge_SF_start == bin_edge_SF_end:
                equal_flag = 1
                mid_pt_to_use = bin_edge_SF_start
            else:
                equal_flag = 0
            #
            ...
            if (diag_flag == 1 and project_diagnostic_flag ==2) or project_diagnostic_flag == 1:
                if project_diagnostic_flag == 0:
                    ...
                    print('Start of : %.2f'%bin_edge_SF_start,' for SF method: %s'%method,' for z_cutoff = %s'%z_cutoff,', ii = %s'%ii)
                    print(bin_edge_SF_start)
                    print(bin_edge_SF_end)
            #
            if bin_edge_SF_start > bin_edge_means_SF[(ii+1)]:   # if the next bin edge is lower than the current bin edge (in mass)...
                ## enable diagnostic output
                if (diag_flag == 1 and project_diagnostic_flag ==2) or project_diagnostic_flag == 1:
                    if project_diagnostic_flag == 0:
                        pass
                ...
                #
                bin_edge_means_SF[(ii+1)] = bin_edge_SF_end
            #
            ##
            while bin_edge_SF_start < bin_edge_SF_end:
                if ii == 0:
                    bin_edge_means_SF[ii] = range3[0]    
                elif ii == (len(bin_edge_means_SF)-1):
                    bin_edge_means_SF[ii] = range3[1]
                else:
                ...
                try:     # try making histograms. if it doesn't work because the bin_edge* array isn't monotonically increasing, raise an error and record a variance of 'NaN'
                    SF_hist, bins_SF = np.histogram(SF_mem, bins=bin_edge_means_SF, range=range3)
                    SF_pos_hist, bins_SF = np.histogram(SF_pos, bins=bin_edge_means_SF, range=range3)
                    SF_neg_hist, bins_SF = np.histogram(SF_neg, bins=bin_edge_means_SF, range=range3)
                    ...
                    SF_var_sum = 0
                    for kk in range(len(SF_ratio)):
                        if SF_ratio[kk] < 1:
                            SF_var_sum = SF_var_sum + (1 - (1/SF_ratio[kk]))**2
                        else:
                            SF_var_sum = SF_var_sum + (1 - SF_ratio[kk])**2
...
#
    ...
    for ii in range(len(bin_edge_means_Q)):
        ...
        #
            Q_var_list = []
            if num_bins_Q[0][ii] < num_bins_Q[1][ii]:    # this if statement won't apply to the first/last bin, since they have been set to equal "range3", the mass range for the study
                if ii == 0:
                    ...
                    bin_edge_Q_end = max(num_bins_Q[1][ii],bin_edge_means_Q[(ii-1)])
                ...
                if ii == 0:
                    ...
                    bin_edge_Q_end = max(num_bins_Q[0][ii],bin_edge_means_Q[(ii-1)])
            else:                                          # for when the i'th bin edge is the same for both false pos/neg lists
            ...
            if bin_edge_Q_start == bin_edge_Q_end:
                equal_flag = 1
                mid_pt_to_use = bin_edge_Q_start
            else:
                equal_flag = 0
            #
            ## enable diagnostic output
            if (diag_flag == 1 and project_diagnostic_flag ==2) or project_diagnostic_flag == 1:
                if project_diagnostic_flag == 0:
                    ...
                    print('Start of : %.2f'%bin_edge_Q_start,' for Q method: %s'%method,' for z_cutoff = %s'%z_cutoff,', ii = %s'%ii)
                    print(bin_edge_Q_start)
                    print(bin_edge_Q_end)
            #
            if bin_edge_Q_start > bin_edge_means_Q[(ii+1)]:   # if the next bin edge is lower than the current bin edge (in mass)...
                ## enable diagnostic output
                if (diag_flag == 1 and project_diagnostic_flag ==2) or project_diagnostic_flag == 1:
                    if project_diagnostic_flag == 0:
                        pass
                        ...
                        print("ERROR 2: the next bin value is lower than the current bin value for cutoffs - spec: %s"%z_cutoff[0],";  phot: %s"%z_cutoff[1],";   for %s"%num_bins_to_try[number]," bins, method = %s"%method,'\nThe current bin is being set to the value of the next bin.\nUSE FEWER BINS')
                #
                bin_edge_means_Q[(ii+1)] = bin_edge_Q_end
            #
            ##
            while bin_edge_Q_start < bin_edge_Q_end:
                if ii == 0:
                    bin_edge_means_Q[ii] = range3[0]
                elif ii == (len(bin_edge_means_Q)-1):
                    bin_edge_means_Q[ii] = range3[1]
                else:
                    bin_edge_means_Q[ii] = bin_edge_Q_start
                try:     # try making histograms. if it doesn't work because the bin_edge* array isn't monotonically increasing, raise an error and record a variance of 'NaN'
                    Q_hist, bins_Q = np.histogram(Q_mem, bins=bin_edge_means_Q, range=range3)
                    Q_pos_hist, bins_Q = np.histogram(Q_pos, bins=bin_edge_means_Q, range=range3)
                    Q_neg_hist, bins_Q = np.histogram(Q_neg, bins=bin_edge_means_Q, range=range3)
                    ...
                    ## compute variance of SF/Q ratios from 1
                    Q_var_sum = 0
                    for kk in range(len(Q_ratio)):
                        if Q_ratio[kk] < 1:
                            Q_var_sum = Q_var_sum + (1 - (1/Q_ratio[kk]))**2
                        else:
                            Q_var_sum = Q_var_sum + (1 - Q_ratio[kk])**2

...
limiting_mass_cluster_field = np.array([0]*6,dtype='float32')
...
for counter in range(len(master_cat)):             # loop through catalogue
    for cluster in range(len(limiting_mag)):
        if master_cat['cluster'][counter] == (cluster+1):       # start by looking at clusters, one at a time
            if master_cat['member'][counter] == 1:    # now look only at cluster FIELD MEMBERS only
...
if summary_flag == 1 or adams_flag == 1:
    ...
    lim_mass_names = Column(['Full CATALOGUE ("master_data*.py")','Phot Members - FIELD','Spec Members - FIELD','Non-members','Bad flux (F160W<0 & F814W<0 & flag == -1)','Mass = NaN (members)','SUM'],name='Property')
...
for ii in range(len(limiting_mag)):
    ...
    if len(masses_at_lim_mag[ii]) == 0:                     # if no objects were found within the tolerance range...
        ...
        limiting_mass_cluster_field[ii] = temp_limit[1]
        ...
        limiting_mass_cluster_field[ii] = masses_at_lim_mag[ii][0][1]
        ...
        limiting_mass_cluster_field[ii] = the_good_shit[1] # store highest mass you have
...
if (diag_flag_2 == 1 and project_diagnostic_flag == 2) or project_diagnostic_flag == 1:
    if project_diagnostic_flag == 0:
        ...
        print('Limiting masses: %s'%limiting_mass_cluster_field)
...
# # range2 = [7.3,12.2]
# if range2_flag == 0:
#     range2=[min(limiting_mass_cluster_field),max_mass[1]]
# elif range2_flag == 1:
#     pass
# #
...
if (plot_flag_1 == 1 and project_plot_flag ==2) or project_plot_flag == 1:
    if project_plot_flag == 0:
        ...
        for cluster in range(len(limiting_mag)):
            ...
            plt.text(20.1,11.5,'$M/L_{max}$: %s'%limiting_mass_cluster_field[cluster],fontsize=10)
        ...
        for cluster in range(len(limiting_mag)):
            ...
            axs.flat[cluster].text(20.1,11.5,'$M/L_{max}$: %s'%limiting_mass_cluster_field[cluster],fontsize=12)
            ...
            #
            #plt.text(21.6,11.5,'$M/L_{max}$: %s'%limiting_mass_cluster_field[cluster],fontsize=10)
        #
...
if (plot_flag_2 == 1 and project_plot_flag ==2) or project_plot_flag == 1:
    if project_plot_flag == 0:
        ...
        axs.flat[0].text(25.15,5.75,'$M/L_{max}$: %s'%limiting_mass_cluster_field[0],fontsize=12)
        axs.flat[1].text(25.15,6.05,cluster_names[1],fontsize=12)
        axs.flat[1].text(25.15,5.75,'$M/L_{max}$: %s'%limiting_mass_cluster_field[1],fontsize=12)
        axs.flat[2].text(25.15,6.05,cluster_names[2],fontsize=12)
        axs.flat[2].text(25.15,5.75,'$M/L_{max}$: %s'%limiting_mass_cluster_field[2],fontsize=12)
        axs.flat[3].text(25.15,6.05,cluster_names[3],fontsize=12)
        axs.flat[3].text(25.15,5.75,'$M/L_{max}$: %s'%limiting_mass_cluster_field[3],fontsize=12)
        axs.flat[4].text(25.15,6.05,cluster_names[4],fontsize=12)
        axs.flat[4].text(25.15,5.75,'$M/L_{max}$: %s'%limiting_mass_cluster_field[4],fontsize=12)
        axs.flat[5].text(25.15,6.05,cluster_names[5],fontsize=12)
        axs.flat[5].text(25.15,5.75,'$M/L_{max}$: %s'%limiting_mass_cluster_field[5],fontsize=12)
...
if (plot_flag_3 == 1 and project_plot_flag ==2) or project_plot_flag == 1:
    if project_plot_flag == 0:
        ...
        for cluster in range(len(limiting_mag)):
            ...
            circle(mag_of_limiting_mass[cluster],limiting_mass_cluster_field[cluster])
...
for counter in range(len(master_cat)):
    for cluster in range(len(below_lim_mass_mem)):
        if master_cat['cluster'][counter] == (cluster+1):              # track galaxies by CLUSTER
            ...
            #     if master_cat['lmass'][counter] < limiting_mass_cluster_field[cluster]:    # isolate galaxies below LIMITING MASS by cluster
            #         master_cat['member'][counter] = 6                  # RECLASSIFY
            #         below_lim_mass_mem[cluster]+=1                     # keep count of numbers
            #     else:
            #         above_lim_mass_mem[cluster]+=1
            if master_cat['member'][counter] == 1:                     # deal with FIELD MEMBERS
                if master_cat['lmass'][counter] < limiting_mass_cluster_field[cluster]:    # isolate galaxies below LIMITING MASS by cluster

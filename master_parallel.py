...
###############     master_parallel
#
##      this file is a carbon copy of the master_data file, except that data 
##      files are read in from the parallel fields. files names are same, 
##      except that "cluster files labelled 'clu', parallel labelled 'par', and
##      the master catalogue has been renamed 'master__cat_par', and all stats
##      tables have the suffix '_par' aswell
#
###############
#
...
z_macs0416 = Table.read('macs0416par_v3.5.zout',format='ascii')
z_macs1149 = Table.read('macs1149par_v3.5.zout',format='ascii')
z_macs0717 = Table.read('macs0717par_v3.5.zout',format='ascii')
z_abell370 = Table.read('abell370par_v3.5.zout',format='ascii')
z_abell1063 = Table.read('abell1063par_v3.5.zout',format='ascii')   
z_abell2744 = Table.read('abell2744par_v3.5.zout',format='ascii')   
#insert other 4 catalogues
#create table from redshift ".fout" file
f_macs0416 = Table.read('macs0416par_v3.5.fout',format='ascii')
f_macs1149 = Table.read('macs1149par_v3.5.fout',format='ascii')
f_macs0717 = Table.read('macs0717par_v3.5.fout',format='ascii')
f_abell370 = Table.read('abell370par_v3.5.fout',format='ascii')
f_abell1063 = Table.read('abell1063par_v3.5.fout',format='ascii')
f_abell2744 = Table.read('abell2744par_v3.5.fout',format='ascii')
#
##read in the whole bloody catalogue to sift for stars
cat_macs0416 = Table.read('hffds_macs0416par_v3.5.cat',format='ascii')
cat_macs1149 = Table.read('hffds_macs1149par_v3.5.cat',format='ascii')
cat_macs0717 = Table.read('hffds_macs0717par_v3.5.cat',format='ascii')
cat_abell370 = Table.read('hffds_abell370par_v3.5.cat',format='ascii')
cat_abell1063 = Table.read('hffds_abell1063par_v3.5.cat',format='ascii')
cat_abell2744 = Table.read('hffds_abell2744par_v3.5.cat',format='ascii')

##creat table for colours
#macs0416
F0416_u = Table.read('macs0416par_v3.5.153.rf',format='ascii')
F0416_v = Table.read('macs0416par_v3.5.155.rf',format='ascii')
F0416_j = Table.read('macs0416par_v3.5.161.rf',format='ascii')
#macs1149
F1149_u = Table.read('macs1149par_v3.5.153.rf',format='ascii')
F1149_v = Table.read('macs1149par_v3.5.155.rf',format='ascii')
F1149_j = Table.read('macs1149par_v3.5.161.rf',format='ascii')
#macs0717
F0717_u = Table.read('macs0717par_v3.5.153.rf',format='ascii')
F0717_v = Table.read('macs0717par_v3.5.155.rf',format='ascii')   
F0717_j = Table.read('macs0717par_v3.5.161.rf',format='ascii')
#abell370
F370_u = Table.read('abell370par_v3.5.153.rf',format='ascii')
F370_v = Table.read('abell370par_v3.5.155.rf',format='ascii')
F370_j = Table.read('abell370par_v3.5.161.rf',format='ascii')
#abell1063
F1063_u = Table.read('abell1063par_v3.5.153.rf',format='ascii')
F1063_v = Table.read('abell1063par_v3.5.155.rf',format='ascii')
F1063_j = Table.read('abell1063par_v3.5.161.rf',format='ascii')
#abell2744
F2744_u = Table.read('abell2744par_v3.5.153.rf',format='ascii')
F2744_v = Table.read('abell2744par_v3.5.155.rf',format='ascii')
F2744_j = Table.read('abell2744par_v3.5.161.rf',format='ascii')
#
...
global data
data_stats_par = Table([data_names,data0,data1,data2,data3,data4,data5,data6])  
#
...
size = len(macs1149)
while counter < size:
    if macs1149[counter]['sub'] ==0 or macs1149[counter]['u'] < 0:
        counter +=1 
    else:
...
master_cat_par.add_column(Column([-99]*len(master_cat_par),name='del_z', dtype=np.float64))                    #<-- z_phot - z_spec / 1 + z_spec
master_cat_par.add_column(Column([-99]*len(master_cat_par),name='z_clusterspec', dtype=np.float64))   #<-- z_spec - z_cl / 1 + z_spec
master_cat_par.add_column(Column([-99]*len(master_cat_par),name='z_clusterphot', dtype=np.float64))   #<-- z_phot - z_cl / 1 + z_phot
#
counter = 0
size = len(master_cat_par)
while counter < size:                       #calucalte del_z for outlier cut
    if master_cat_par[counter]['sub'] == 1:
        master_cat_par[counter]['del_z'] = ((master_cat_par[counter]['z_peak'] - master_cat_par[counter]['z_spec']) / (1 + master_cat_par[counter]['z_spec']))
        counter +=1
    else:
        counter +=1
#
...
counter = 0
while counter < size: 
    if master_cat_par[counter]['sub'] == 2 and master_cat_par[counter]['sub'] != 0:
        phot_subsample +=1
    elif master_cat_par[counter]['sub'] == 1 and master_cat_par[counter]['sub'] != 0:
        if abs(master_cat_par[counter]['del_z']) > 0.15: #identify objects w/ del_z > 0.15
            master_cat_par[counter]['type'] = 3
            outliers += 1
            if master_cat_par[counter]['cluster'] == 1:
                    out[0][0] +=1
                    both[0][0] -=1
            elif master_cat_par[counter]['cluster'] == 2:
                    out[0][1] +=1
                    both[0][1] -=1
            elif master_cat_par[counter]['cluster'] == 3:
                    out[0][2] +=1
                    both[0][2] -=1
            elif master_cat_par[counter]['cluster'] == 4:
                    out[0][3] +=1
                    both[0][3] -=1
            elif master_cat_par[counter]['cluster'] == 5:
                    out[0][4] +=1
                    both[0][4] -=1
            elif master_cat_par[counter]['cluster'] == 6:
                    out[0][5] +=1
                    both[0][5] -=1
        else: 
...
size = len(master_cat_par)
while counter < size:
    if master_cat_par[counter]['sub'] == 1:         #spec sample del_z plot for cuts: denominator = (1 + z_spec)
        if master_cat_par[counter]['cluster'] == 1:
            master_cat_par[counter]['z_clusterspec'] = ((master_cat_par[counter]['z_spec'] - z_cluster[0]) / (1 + master_cat_par[counter]['z_spec']))
            master_cat_par[counter]['z_clusterphot'] = ((master_cat_par[counter]['z_peak'] - z_cluster[0]) / (1 + master_cat_par[counter]['z_peak']))
        elif master_cat_par[counter]['cluster'] == 2:
            master_cat_par[counter]['z_clusterspec'] = ((master_cat_par[counter]['z_spec'] - z_cluster[1]) / (1 + master_cat_par[counter]['z_spec']))
            master_cat_par[counter]['z_clusterphot'] = ((master_cat_par[counter]['z_peak'] - z_cluster[1]) / (1 + master_cat_par[counter]['z_peak']))
        elif master_cat_par[counter]['cluster'] == 3:
            master_cat_par[counter]['z_clusterspec'] = (master_cat_par[counter]['z_spec'] - z_cluster[2]) / (1 + master_cat_par[counter]['z_spec'])
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[2]) / (1 + master_cat_par[counter]['z_peak'])
        elif master_cat_par[counter]['cluster'] == 4:
            master_cat_par[counter]['z_clusterspec'] = (master_cat_par[counter]['z_spec'] - z_cluster[3]) / (1 + master_cat_par[counter]['z_spec'])
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[3]) / (1 + master_cat_par[counter]['z_peak'])
        elif master_cat_par[counter]['cluster'] == 5:
            master_cat_par[counter]['z_clusterspec'] = (master_cat_par[counter]['z_spec'] - z_cluster[4]) / (1 + master_cat_par[counter]['z_spec'])
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[4]) / (1 + master_cat_par[counter]['z_peak'])
        elif master_cat_par[counter]['cluster'] == 6:
            master_cat_par[counter]['z_clusterspec'] = (master_cat_par[counter]['z_spec'] - z_cluster[5]) / (1 + master_cat_par[counter]['z_spec'])
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[5]) / (1 + master_cat_par[counter]['z_peak'])
        counter +=1
    elif master_cat_par[counter]['sub'] == 2:       #phot sample del_z plot for cuts defined in v7a to be (z_phot - z_cl)
        if master_cat_par[counter]['cluster'] == 1:
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[0]) / (1 + z_cluster[0])
        elif master_cat_par[counter]['cluster'] == 2:
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[1]) / (1 + z_cluster[1])
        elif master_cat_par[counter]['cluster'] == 3:
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[2]) / (1 + z_cluster[2])
        elif master_cat_par[counter]['cluster'] == 4:
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[3]) / (1 + z_cluster[3])
        elif master_cat_par[counter]['cluster'] == 5:
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[4]) / (1 + z_cluster[4])
        elif master_cat_par[counter]['cluster'] == 6:
            master_cat_par[counter]['z_clusterphot'] = (master_cat_par[counter]['z_peak'] - z_cluster[5]) / (1 + z_cluster[5]) 
        counter +=1
    else: 
        counter +=1
#
...
counter = 0
while counter < size: 
    if master_cat_par[counter]['star_flag'] == 1: #identify objects with a "star flag" = 1 for stars; gal=0, mag>25 = 2
        master_cat_par[counter]['type'] = 0
        if master_cat_par[counter]['sub'] == 0:     #keep track of which subsample stars came from
            star_type[0][0] +=1
        elif master_cat_par[counter]['sub'] == 1:   #spec stars
            spec_subsample -=1
            star_type[0][1] +=1
        elif master_cat_par[counter]['sub'] ==2:    #phot stars
            phot_subsample -=1
            star_type[0][2] +=1
        if master_cat_par[counter]['cluster'] == 1:
            star[0][0] +=1
...
counter = 0
while counter < size: 
    if master_cat_par[counter]['sub'] == 1 and master_cat_par[counter]['type'] != 3 and master_cat_par[counter]['type'] != 0:        #spec subsample, skipping stars & outliers
        if master_cat_par[counter]['uv'] > 1.3 and master_cat_par[counter]['vj'] < 1.6 and master_cat_par[counter]['uv'] > ((0.88*master_cat_par[counter]['vj']) + 0.6): #identify passive (quiescent)
            master_cat_par[counter]['type'] = 2
#            spec_Q +=1
            if master_cat_par[counter]['cluster'] == 1:
                Q_spec[0][0] +=1
            elif master_cat_par[counter]['cluster'] == 2:
            ...
            master_cat_par[counter]['type'] = 1         #if not Q, then must be SF
#            spec_SF +=1
            if master_cat_par[counter]['cluster'] == 1:
                SF_spec[0][0] +=1
            elif master_cat_par[counter]['cluster'] == 2:
                ...
                SF_spec[0][5] +=1  
    elif master_cat_par[counter]['sub'] == 2 and master_cat_par[counter]['type'] != 3 and master_cat_par[counter]['type'] != 0:               #phot subsample
        if master_cat_par[counter]['uv'] > 1.3 and master_cat_par[counter]['vj'] < 1.6 and master_cat_par[counter]['uv'] > ((0.88*master_cat_par[counter]['vj'] + 0.6)): #identify passive (quiescent)
            master_cat_par[counter]['type'] = 2
#            phot_Q +=1
            if master_cat_par[counter]['cluster'] == 1:
...
#            phot_SF +=1
            if master_cat_par[counter]['cluster'] == 1:
                ...
                SF_phot[0][5] +=1   
    elif master_cat_par[counter]['type'] == 0:               #stars
        check1 +=1
    else: 
...
macros_type_par = Table([macro_type,tot,sp,ph],names=('Property','Total','spec','phot'))
#
#
...
counter = 0
while counter < size:
    if master_cat_par[counter]['sub'] == 1:       # ['sub'] == 1: spec
        if master_cat_par[counter]['type'] == 0 or master_cat_par[counter]['type'] ==3:    #['type'] == 0: stars,    ['type'] == 3: spec_only
            other +=1
        elif master_cat_par[counter]['type'] == 1:      # '[type'] == 1:   look only at SF spec population
            if abs(master_cat_par[counter]['z_clusterspec']) > 0.01 and abs(master_cat_par[counter]['z_clusterphot']) > 0.1:  # secure field
                master_cat_par[counter]['member'] = 1   #field
                if master_cat_par[counter]['cluster'] == 1:
                    sfield_par[0][0] +=1
                elif master_cat_par[counter]['cluster'] == 2:
                    sfield_par[0][1] +=1
                    ...
                    sfield_par[0][5] +=1  
            elif abs(master_cat_par[counter]['z_clusterspec']) > 0.01 and abs(master_cat_par[counter]['z_clusterphot']) < 0.03:  #  false positive
                master_cat_par[counter]['member'] = 2   #false positive
                if master_cat_par[counter]['cluster'] == 1:
                    spos[0][0] +=1
                elif master_cat_par[counter]['cluster'] == 2:
                    spos[0][1] +=1
                    ...
                    spos[0][5] +=1
            elif abs(master_cat_par[counter]['z_clusterspec']) < 0.01 and abs(master_cat_par[counter]['z_clusterphot']) > 0.03:  #  false negative
                master_cat_par[counter]['member'] = 3   #false negative
                if master_cat_par[counter]['cluster'] == 1:
                    sneg[0][0] +=1
                elif master_cat_par[counter]['cluster'] == 2:
                    sneg[0][1] +=1
                    ...
                    sneg[0][5] +=1
            elif abs(master_cat_par[counter]['z_clusterspec']) < 0.01 and abs(master_cat_par[counter]['z_clusterphot']) < 0.03:
                master_cat_par[counter]['member'] = 0              #secure cluster member
                if master_cat_par[counter]['cluster'] == 1:
                    smem_par[0][0] +=1
                elif master_cat_par[counter]['cluster'] == 2:
                    smem_par[0][1] +=1
                    ...
                    smem_par[0][5] +=1 
        elif master_cat_par[counter]['type'] == 2:          
            other+=1
            if abs(master_cat_par[counter]['z_clusterspec']) > 0.01 and abs(master_cat_par[counter]['z_clusterphot']) > 0.1:  # secure field
                master_cat_par[counter]['member'] = 1       #field
                if master_cat_par[counter]['cluster'] == 1:
                    ...
                    qfield_par[0][5] +=1  
            elif abs(master_cat_par[counter]['z_clusterspec']) > 0.01 and abs(master_cat_par[counter]['z_clusterphot']) < 0.07:  #  false positive
                master_cat_par[counter]['member'] = 2       #false positive
                if master_cat_par[counter]['cluster'] == 1:
                    qpos[0][0] +=1
                elif master_cat_par[counter]['cluster'] == 2:
                    ...
                    qpos[0][5] +=1
            elif abs(master_cat_par[counter]['z_clusterspec']) < 0.01 and abs(master_cat_par[counter]['z_clusterphot']) > 0.07:  #  false negative
                master_cat_par[counter]['member'] = 3       #false negative
                if master_cat_par[counter]['cluster'] == 1:
                    qneg[0][0] +=1
                elif master_cat_par[counter]['cluster'] == 2:
                    ...
                    qneg[0][5] +=1
            elif abs(master_cat_par[counter]['z_clusterspec']) < 0.01 and abs(master_cat_par[counter]['z_clusterphot']) < 0.07:
                master_cat_par[counter]['member'] = 0              #secure cluster member
                if master_cat_par[counter]['cluster'] == 1:
                    qmem_par[0][0] +=1
                elif master_cat_par[counter]['cluster'] == 2:
...
total = Column([len(master_cat_par),np.sum(nodata),spec_subsample,phot_subsample,np.sum(star),np.sum(SF_spec)+np.sum(SF_phot),np.sum(SF_spec),np.sum(SF_phot),np.sum(Q_spec)+np.sum(Q_phot),np.sum(Q_spec),np.sum(Q_phot),outliers])
...
macros_par1 = Table([macro_names,total,cl1,cl2],names=('Property','Full catalogue','1.macs0416','2.macs1149'))
macros_par2 = Table([macro_names,cl3,cl4,cl5,cl6],names=('Property','3.macs0717','4.abell370','5.abell1063','6.abel2744'))
#
...
total_mems = Column([np.sum(smem_par),np.sum(sfield_par),np.sum(spos),np.sum(sneg),np.sum([smem_par,sfield_par,spos,sneg]),np.sum(qmem_par),np.sum(qfield_par),np.sum(qpos),np.sum(qneg),np.sum([qmem_par,qfield_par,qpos,qneg]),np.sum([smem_par,sfield_par,spos,sneg,qmem_par,qfield_par,qpos,qneg])])
cl1_mems = Column([smem_par[0][0],sfield_par[0][0],spos[0][0],sneg[0][0],np.sum([smem_par[0][0],sfield_par[0][0],spos[0][0],sneg[0][0]]),qmem_par[0][0],qfield_par[0][0],qpos[0][0],qneg[0][0],np.sum([qmem_par[0][0],qfield_par[0][0],qpos[0][0],qneg[0][0]]),np.sum([smem_par[0][0],sfield_par[0][0],spos[0][0],sneg[0][0],qmem_par[0][0],qfield_par[0][0],qpos[0][0],qneg[0][0]])])
cl2_mems = Column([smem_par[0][1],sfield_par[0][1],spos[0][1],sneg[0][1],np.sum([smem_par[0][1],sfield_par[0][1],spos[0][1],sneg[0][1]]),qmem_par[0][1],qfield_par[0][1],qpos[0][1],qneg[0][1],np.sum([qmem_par[0][1],qfield_par[0][1],qpos[0][1],qneg[0][1]]),np.sum([smem_par[0][1],sfield_par[0][1],spos[0][1],sneg[0][1],qmem_par[0][1],qfield_par[0][1],qpos[0][1],qneg[0][1]])])
cl3_mems = Column([smem_par[0][2],sfield_par[0][2],spos[0][2],sneg[0][2],np.sum([smem_par[0][2],sfield_par[0][2],spos[0][2],sneg[0][2]]),qmem_par[0][2],qfield_par[0][2],qpos[0][2],qneg[0][2],np.sum([qmem_par[0][2],qfield_par[0][2],qpos[0][2],qneg[0][2]]),np.sum([smem_par[0][2],sfield_par[0][2],spos[0][2],sneg[0][2],qmem_par[0][2],qfield_par[0][2],qpos[0][2],qneg[0][2]])])
cl4_mems = Column([smem_par[0][3],sfield_par[0][3],spos[0][3],sneg[0][3],np.sum([smem_par[0][3],sfield_par[0][3],spos[0][3],sneg[0][3]]),qmem_par[0][3],qfield_par[0][3],qpos[0][3],qneg[0][3],np.sum([qmem_par[0][3],qfield_par[0][3],qpos[0][3],qneg[0][3]]),np.sum([smem_par[0][3],sfield_par[0][3],spos[0][3],sneg[0][3],qmem_par[0][3],qfield_par[0][3],qpos[0][3],qneg[0][3]])])
cl5_mems = Column([smem_par[0][4],sfield_par[0][4],spos[0][4],sneg[0][4],np.sum([smem_par[0][4],sfield_par[0][4],spos[0][4],sneg[0][4]]),qmem_par[0][4],qfield_par[0][4],qpos[0][4],qneg[0][4],np.sum([qmem_par[0][4],qfield_par[0][4],qpos[0][4],qneg[0][4]]),np.sum([smem_par[0][4],sfield_par[0][4],spos[0][4],sneg[0][4],qmem_par[0][4],qfield_par[0][4],qpos[0][4],qneg[0][4]])])
cl6_mems = Column([smem_par[0][5],sfield_par[0][5],spos[0][5],sneg[0][5],np.sum([smem_par[0][5],sfield_par[0][5],spos[0][5],sneg[0][5]]),qmem_par[0][5],qfield_par[0][5],qpos[0][5],qneg[0][5],np.sum([qmem_par[0][5],qfield_par[0][5],qpos[0][5],qneg[0][5]]),np.sum([smem_par[0][5],sfield_par[0][5],spos[0][5],sneg[0][5],qmem_par[0][5],qfield_par[0][5],qpos[0][5],qneg[0][5]])])

spec_stats_par1 = Table([spec_names,total_mems,cl1_mems,cl2_mems],names=('Property','Full catalogue','1.macs0416','2.macs1149'))
spec_stats_par2 = Table([spec_names,cl3_mems,cl4_mems,cl5_mems,cl6_mems],names=('Property','3.macs0717','4.abell370','5.abell1063','6.abel2744'))
#
#
#
#
...
counter = 0
while counter < size:
    if master_cat_par[counter]['sub'] ==2:
        if master_cat_par[counter]['type'] == 0 or master_cat_par[counter]['type'] ==3: #skip stars and outliers
            other +=1       
        elif master_cat_par[counter]['type'] ==1:       # SF
            aa[0][0] +=1
            if abs(master_cat_par[counter]['z_clusterphot']) > 0.05:     #phot field
                aa[0][1] +=1
                if master_cat_par[counter]['z_peak'] >0.55 or master_cat_par[counter]['z_peak'] <0.3:
                    master_cat_par[counter]['member'] = 4
                else:
                    master_cat_par[counter]['member'] = 1
                    if master_cat_par[counter]['cluster'] == 1:
                        phot_fields_par[0][0] +=1
                    elif master_cat_par[counter]['cluster'] == 2:
                        phot_fields_par[0][1] +=1
                    elif master_cat_par[counter]['cluster'] == 3:
                        phot_fields_par[0][2] +=1    
                    elif master_cat_par[counter]['cluster'] == 4:
                        phot_fields_par[0][3] +=1    
                    elif master_cat_par[counter]['cluster'] == 5:
                        phot_fields_par[0][4] +=1    
                    elif master_cat_par[counter]['cluster'] == 6:
                        phot_fields_par[0][5] +=1
            else:
                master_cat_par[counter]['member'] = 0
                if master_cat_par[counter]['cluster'] == 1:
                    phot_mems_par[0][0] +=1
                elif master_cat_par[counter]['cluster'] == 2:
                    phot_mems_par[0][1] +=1
                    ...
                    phot_mems_par[0][5] +=1
        elif master_cat_par[counter]['type'] ==2:       #Q
            if abs(master_cat_par[counter]['z_clusterphot']) > 0.05:     #phot field
                if master_cat_par[counter]['z_peak'] >0.55 or master_cat_par[counter]['z_peak'] <0.3:
                    master_cat_par[counter]['member'] = 4
                else:
                    master_cat_par[counter]['member'] = 1       #field
                    if master_cat_par[counter]['cluster'] == 1:
                        phot_fieldq_par[0][0] +=1
                    elif master_cat_par[counter]['cluster'] == 2:
                        phot_fieldq_par[0][1] +=1
                    elif master_cat_par[counter]['cluster'] == 3:
                        phot_fieldq_par[0][2] +=1    
                    elif master_cat_par[counter]['cluster'] == 4:
                        phot_fieldq_par[0][3] +=1    
                    elif master_cat_par[counter]['cluster'] == 5:
                        phot_fieldq_par[0][4] +=1    
                    elif master_cat_par[counter]['cluster'] == 6:
                        phot_fieldq_par[0][5] +=1
            else:
                master_cat_par[counter]['member'] = 0       #cluster member, phot
                if master_cat_par[counter]['cluster'] == 1:
                    phot_memq_par[0][0] +=1
                elif master_cat_par[counter]['cluster'] == 2:
                    phot_memq_par[0][1] +=1
...
total_memp = Column([np.sum(phot_mems_par),np.sum(phot_fields_par),np.sum([phot_mems_par,phot_fields_par]),np.sum(phot_memq_par),np.sum(phot_fieldq_par),np.sum([phot_memq_par,phot_fieldq_par]),np.sum([phot_mems_par,phot_fields_par,phot_memq_par,phot_fieldq_par])])
cl1_memp = Column([phot_mems_par[0][0],phot_fields_par[0][0],np.sum([phot_mems_par[0][0],phot_fields_par[0][0]]),phot_memq_par[0][0],phot_fieldq_par[0][0],np.sum([phot_memq_par[0][0],phot_fieldq_par[0][0]]),np.sum([phot_mems_par[0][0],phot_fields_par[0][0],phot_memq_par[0][0],phot_fieldq_par[0][0]])])
cl2_memp = Column([phot_mems_par[0][1],phot_fields_par[0][1],np.sum([phot_mems_par[0][1],phot_fields_par[0][1]]),phot_memq_par[0][1],phot_fieldq_par[0][1],np.sum([phot_memq_par[0][1],phot_fieldq_par[0][1]]),np.sum([phot_mems_par[0][1],phot_fields_par[0][1],phot_memq_par[0][1],phot_fieldq_par[0][1]])])
cl3_memp = Column([phot_mems_par[0][2],phot_fields_par[0][2],np.sum([phot_mems_par[0][2],phot_fields_par[0][2]]),phot_memq_par[0][2],phot_fieldq_par[0][2],np.sum([phot_memq_par[0][2],phot_fieldq_par[0][2]]),np.sum([phot_mems_par[0][2],phot_fields_par[0][2],phot_memq_par[0][2],phot_fieldq_par[0][2]])])
cl4_memp = Column([phot_mems_par[0][3],phot_fields_par[0][3],np.sum([phot_mems_par[0][3],phot_fields_par[0][3]]),phot_memq_par[0][3],phot_fieldq_par[0][3],np.sum([phot_memq_par[0][3],phot_fieldq_par[0][3]]),np.sum([phot_mems_par[0][3],phot_fields_par[0][3],phot_memq_par[0][3],phot_fieldq_par[0][3]])])
cl5_memp = Column([phot_mems_par[0][4],phot_fields_par[0][4],np.sum([phot_mems_par[0][4],phot_fields_par[0][4]]),phot_memq_par[0][4],phot_fieldq_par[0][4],np.sum([phot_memq_par[0][4],phot_fieldq_par[0][4]]),np.sum([phot_mems_par[0][4],phot_fields_par[0][4],phot_memq_par[0][4],phot_fieldq_par[0][4]])])
cl6_memp = Column([phot_mems_par[0][5],phot_fields_par[0][5],np.sum([phot_mems_par[0][5],phot_fields_par[0][5]]),phot_memq_par[0][5],phot_fieldq_par[0][5],np.sum([phot_memq_par[0][5],phot_fieldq_par[0][5]]),np.sum([phot_mems_par[0][5],phot_fields_par[0][5],phot_memq_par[0][5],phot_fieldq_par[0][5]])])
phot_stats_par1 = Table([phot_names,total_memp,cl1_memp,cl2_memp],names=('Property','Full catalogue','1.macs0416','2.macs1149'))
phot_stats_par2 = Table([phot_names,cl3_memp,cl4_memp,cl5_memp,cl6_memp],names=('Property','3.macs0717','4.abell370','5.abell1063','6.abel2744'))
#
...
BCG_counter = 0
while counter < size: 
    if master_cat_par[counter]['BCG'] == 2: #per .cat documentation, "use_{KS/CH1/CH2}" ==2 identifies BCGs. have confirmed each tracks the same # of BCGs per cluster
        BCG.add_row(master_cat_par[counter])
        if master_cat_par[counter]['cluster'] == 1:
            BCG_cl[0][0] +=1
        elif master_cat_par[counter]['cluster'] == 2:
            BCG_cl[0][1] +=1
            ...
            BCG_cl[0][5] +=1
        if master_cat_par[counter]['sub'] ==0:
            BCG_sub[0][0]+=1
        elif master_cat_par[counter]['sub'] ==1:
            BCG_sub[0][1]+=1
        elif master_cat_par[counter]['sub'] ==2:
            BCG_sub[0][2]+=1
        elif master_cat_par[counter]['sub'] ==3:
            BCG_sub[0][3]+=1
        if master_cat_par[counter]['member'] == 0:
            BCG_mem[0][0]+=1
        elif master_cat_par[counter]['member'] == 1:
            BCG_mem[0][1]+=1
        elif master_cat_par[counter]['member'] == 2:
            BCG_mem[0][2]+=1
        elif master_cat_par[counter]['member'] == 3:
            BCG_mem[0][3]+=1
    counter +=1
#
## 
...
members = Column([np.sum(phot_mems_par)+np.sum(smem_par),np.sum(phot_mems_par),np.sum(smem_par),np.sum(spos),np.sum(sneg),np.sum(spos)/np.sum(sneg),np.sum(phot_memq_par)+np.sum(qmem_par),np.sum(phot_memq_par),np.sum(qmem_par),np.sum(qpos),np.sum(qneg),np.sum(qpos)/np.sum(qneg),np.sum(phot_mems_par)+np.sum(smem_par)+np.sum(phot_memq_par)+np.sum(qmem_par)])
field = Column([np.sum(phot_fields_par)+np.sum(sfield_par),np.sum(phot_fields_par),np.sum(sfield_par),na,na,na,np.sum(phot_fieldq_par)+np.sum(qfield_par),np.sum(phot_fieldq_par),np.sum(qfield_par),na,na,na,np.sum(phot_fields_par)+np.sum(sfield_par)+np.sum(phot_fieldq_par)+np.sum(qfield_par)])
sample_stats_par = Table([sample_names,members,field],names=('Property','Selected members','"Secure" field'))

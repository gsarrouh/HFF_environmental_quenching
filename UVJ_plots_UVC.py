...
field_array = [ [], [], [] ]
## cluster
for counter in range(len(master_cat_uvc_uvc)):
    if master_cat_uvc['member'][counter] == 0:                   # field galaxies
        counting_array_uvj[0]+=1
        field_array[0].append(master_cat_uvc['vj'][counter])
        field_array[1].append(master_cat_uvc['uv'][counter])
        field_array[2].append(master_cat_uvc['lmass'][counter])
        total_array[0].append(master_cat_uvc['vj'][counter])
        total_array[1].append(master_cat_uvc['uv'][counter])
        total_array[2].append(master_cat_uvc['lmass'][counter])
    elif master_cat_uvc['member'][counter] == 11:                 # members
        if master_cat_uvc['type'][counter] == 1:
            counting_array_uvj[1]+=1                         # count for SF
            SF_array[0].append(master_cat_uvc['vj'][counter])
            SF_array[1].append(master_cat_uvc['uv'][counter])
            SF_array[2].append(master_cat_uvc['lmass'][counter])
            total_array[0].append(master_cat_uvc['vj'][counter])
            total_array[1].append(master_cat_uvc['uv'][counter])
            total_array[2].append(master_cat_uvc['lmass'][counter])
        elif master_cat_uvc['type'][counter]== 2:
            counting_array_uvj[2]+=1                         # count for Q
            Q_array[0].append(master_cat_uvc['vj'][counter])
            Q_array[1].append(master_cat_uvc['uv'][counter])
            Q_array[2].append(master_cat_uvc['lmass'][counter])
            total_array[0].append(master_cat_uvc['vj'][counter])
            total_array[1].append(master_cat_uvc['uv'][counter])
            total_array[2].append(master_cat_uvc['lmass'][counter])
        else:
            counting_array_uvj[3]+=1
        if master_cat_uvc['sub'][counter] == 1:
            counting_array_sub[0]+=1                         # count for spec+phot subsample
            spec_array[0].append(master_cat_uvc['vj'][counter])
            spec_array[1].append(master_cat_uvc['uv'][counter])
        elif master_cat_uvc['sub'][counter] == 2:
            counting_array_sub[1]+=1                         # count for phot-only subsample
            phot_array[0].append(master_cat_uvc['vj'][counter])
            phot_array[1].append(master_cat_uvc['uv'][counter])
    else:
...
# #
# for counter in range(len(master_cat_uvc_par)):
#     if master_cat_uvc_par['member'][counter] == 1:                   # field galaxies
#         counting_array_par[0]+=1
#         field_array_par[0].append(master_cat_uvc_par['vj'][counter])
#         field_array_par[1].append(master_cat_uvc_par['uv'][counter])
#         field_array_par[2].append(master_cat_uvc_par['lmass'][counter])
#     else:
...
if (plot_flag_1 == 1 and project_plot_flag ==2) or project_plot_flag == 1:
    if project_plot_flag == 0:
        ...
        sm.set_array([])
        sm.set_clim([min(min(total_array[2]),min(field_array[2])),max(max(total_array[2]),max(field_array[2]))])
        ...
        ax.text(-1.3,2.0,'Field (clu+par): %s'%(counting_array_uvj[0]),c='k',fontsize=25)
        #
        plt.show()
        #
    #
...
if (plot_flag_2 == 1 and project_plot_flag ==2) or project_plot_flag == 1:
    if project_plot_flag == 0:
        ...
        ax_histx.hist(np.concatenate([total_array_hi_mass[0],field_array_hi_mass[0]]), bins=bins_x, color='slategrey')
        ax_histy.hist(np.concatenate([total_array_hi_mass[1],field_array_hi_mass[1]]), bins=bins_y, color='slategrey', orientation='horizontal')
        #
...
#####
    ...
    if project_plot_flag == 0:
        ...
        bins_x = np.arange(-1.5, 2, binwidth)
        bins_y = np.arange(0, 2.5, binwidth)
        ax_histx.hist(np.concatenate([total_array[0],field_array[0]]), bins=bins_x, color='slategrey')
        ax_histy.hist(np.concatenate([total_array[1],field_array[1]]), bins=bins_y, color='slategrey', orientation='horizontal')
        #
        ax_histx.set_xlim(ax_scatter.get_xlim())
        ax_histy.set_ylim(ax_scatter.get_ylim())
        #
        ...
        ax_scatter.text(-1.3,2.0,'Field (clu+par): %s'%(counting_array_uvj[0]),c='k',fontsize=15)
        #
        plt.show()
        #
        #
...
if (plot_flag_3 == 1 and project_plot_flag ==2) or project_plot_flag == 1:
    if project_plot_flag == 0:
        ...
        ax.text(-1.3,2.0,'Field (clu+par): %s'%(counting_array_uvj[0]),c='k',fontsize=25,color='grey')
        #
        plt.show()
        #
    #

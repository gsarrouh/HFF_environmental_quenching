...
### Each section calls a program file (i.e. *.py script file) which accomplishes a task. Some programs call other programs. They are all listed in the Section summary which follows. This program provides the main organizational framework for the project. To find a section, search "SECTION (*)" in all caps, where * is the section number you want. To find fields which require user input, search "MAY NEED TO EDIT" in all caps. Some of these fields may direct you to manually enter input into a sub-program. 
#
#
### Section summary:
#
...
#
### (1)    main DATA preparation file; imports raw data, sorts it by data type, and classifies them as SF/Q; runs a VARIATIONAL ANALYSIS to determine optimal definition of cluster membership; executes redshift cuts, classifies SF/Q galaxies as cluster members; finally, checks the catalogue for Brightest Cluster Galaxies (bCGs); MAIN program: master_data_7_final.py; SUB-programs: spec_completeness_binning.py, correction_factors.py; 
#
### (2):   REDSHIFT plots; executes a file which produces figures ANALYZING the QUALITY of the data set; MAIN program: master_zplots_2_final.py;
#
### (3):   determine LIMITING MASS; MAIN program: ******.py 
#
### (4):   produce Stellar Mass Function (SMF); MAIN program: master_smfz_8_final.py;
#
### (5):   ;
#
### (6):   ;

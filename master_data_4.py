...
while counter < size:
    if master_cat[counter]['sub'] == 1:       # ['sub'] == 1: spec
        if master_cat[counter]['type'] == 0 or master_cat[counter]['type'] ==3:    #['type'] == 0: stars,    ['type'] == 3: spec_only
            ...
            if abs(master_cat[counter]['z_clusterspec']) > 0.01 and abs(master_cat[counter]['z_clusterphot']) > 0.03:  # secure field
            ...
            if abs(master_cat[counter]['z_clusterspec']) > 0.01 and abs(master_cat[counter]['z_clusterphot']) > 0.07:  # secure field
                master_cat[counter]['member'] = 1       #field
                if master_cat[counter]['cluster'] == 1:
                    qfield[0][0] +=1
                elif master_cat[counter]['cluster'] == 2:
